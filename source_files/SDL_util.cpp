		...
		while (SDL_PollEvent(&this->event)) {
			if (!( this->exitProgram = callback(this->event))) {
				if (this->callbackList[( SDL_EventType ) this->event.type])
					this->callbackList[( SDL_EventType ) this->event.type]->forEach([&](int index, int id, std::function<void(SDL_Event)> &&eventCallback) {
...
EventManager *EventManager::onGameTick(SDL_EventType eventType, std::function<void(SDL_Event)> &&callback) {
	if (!this->gameTickCallbackList[eventType]) this->gameTickCallbackList[eventType] = new Array<std::function<void(SDL_Event)>>;
...
void EventManager::gameTick( ) {
	if (this->gameTickCallbackList[( SDL_EventType ) this->event.type]) {
		this->gameTickCallbackList[( SDL_EventType ) this->event.type]->forEach([&](int index, int id, std::function<void(SDL_Event)> &&tickCallback) {
